...
test('Average', t => {
    const grades = new List([78, 92, 100, 37, 81]);
...
test('Distinct', t => {
    const ages = new List([21, 46, 46, 55, 17, 21, 55, 55]);
    const pets = new List([
        new Pet({ Age: 1, Name: 'Whiskers' }),
        new Pet({ Age: 1, Name: 'Whiskers' }),
        new Pet({ Age: 8, Name: 'Barley' }),
...
test('ElementAt', t => {
    const a = new List(['hey', 'hola', 'que', 'tal']);
    ...
    t.throws(() => a.ElementAt(-1), /ArgumentOutOfRangeException: index is less than 0 or greater than or equal to the number of elements in source./);
});
test('ElementAtOrDefault', t => {
    const a = new List(['hey', 'hola', 'que', 'tal']);
    const b = new List([2, 1, 0, -1, -2]);
...
test('Except', t => {
    const numbers1 = new List([2.0, 2.1, 2.2, 2.3, 2.4, 2.5]);
    const numbers2 = new List([2.2, 2.3]);
    t.deepEqual(numbers1.Except(numbers2).ToArray(), [2, 2.1, 2.4, 2.5]);
});
test('First', t => {
    t.is(new List(['hey', 'hola', 'que', 'tal']).First(), 'hey');
    t.is(new List([1, 2, 3, 4, 5]).First(x => x > 2), 3);
    t.throws(() => new List().First(), /InvalidOperationException: The source sequence is empty./);
});
test('FirstOrDefault', t => {
    t.is(new List(['hey', 'hola', 'que', 'tal']).FirstOrDefault(), 'hey');
    t.is(new List().FirstOrDefault(), undefined);
});
test('ForEach', t => {
    const names = new List(['Bruce', 'Alfred', 'Tim', 'Richard']);
    let test = '';
...
test('GroupJoin', t => {
    ...
    const people = new List([magnus, terry, charlotte]);
    ...
    t.deepEqual(query.Select(obj => `${obj.OwnerName}: ${obj.Pets.ToArray()}`).ToArray(), expected);
});
test('IndexOf', t => {
    const fruits = new List([
        'apple',
    ...
    const pets = new List([barley, boots, whiskers]);
...
test('Intersect', t => {
    const id1 = new List([44, 26, 92, 30, 71, 38]);
    const id2 = new List([39, 59, 83, 47, 26, 4, 30]);
...
test('Join', t => {
    ...
    const query = people.Join(pets, person => person, pet => pet.Owner, (person, pet) => ({ OwnerName: person.Name, Pet: pet.Name }));
    const expected = [
        'Hedlund, Magnus - Daisy',
        'Adams, Terry - Barley',
        'Adams, Terry - Boots',
    ...
    t.deepEqual(query.Select(obj => `${obj.OwnerName} - ${obj.Pet}`).ToArray(), expected);
});
test('Last', t => {
    t.is(new List(['hey', 'hola', 'que', 'tal']).Last(), 'tal');
    t.is(new List([1, 2, 3, 4, 5]).Last(x => x > 2), 5);
    t.throws(() => new List().Last(), /InvalidOperationException: The source sequence is empty./);
});
test('LastOrDefault', t => {
    t.is(new List(['hey', 'hola', 'que', 'tal']).LastOrDefault(), 'tal');
    t.is(new List().LastOrDefault(), undefined);
});
test('Max', t => {
    const people = new List([
        { Age: 15, Name: 'Cathy' },
...
test('OfType', t => {
    ...
    const anyArray = new List(['dogs', 'cats', 13, true]);
...
test('OrderBy', t => {
    ...
    t.deepEqual(new List([4, 5, 6, 3, 2, 1]).OrderBy(x => x).ToArray(), expected);
...
test('OrderByDescending', t => {
    t.deepEqual(new List([4, 5, 6, 3, 2, 1]).OrderByDescending(x => x).ToArray(), [6, 5, 4, 3, 2, 1]);
});
test('ThenBy', t => {
    const fruits = new List([
        'grape',
    ...
    t.deepEqual(new List([4, 5, 6, 3, 2, 1]).ThenBy(x => x).ToArray(), expectedNums);
});
...
test('ThenByDescending', t => {
    ...
    t.deepEqual(fruits
        ...
        .ToArray(), expected);
    t.deepEqual(new List([4, 5, 6, 3, 2, 1]).ThenByDescending(x => x).ToArray(), [6, 5, 4, 3, 2, 1]);
});
test('Remove', t => {
    const fruits = new List([
        'apple',
    ...
    const whiskers = new Pet({ Age: 1, Name: 'Whiskers', Vaccinated: false });
    const pets = new List([barley, boots, whiskers]);
    const lessPets = new List([barley, whiskers]);
...
test('Reverse', t => {
    t.deepEqual(new List([1, 2, 3, 4, 5]).Reverse().ToArray(), [
        5,
        4,
        3,
        2,
...
test('Select', t => {
    t.deepEqual(new List([1, 2, 3]).Select(x => x * 2).ToArray(), [
        2,
        4,
        6
    ]);
...
test('SequenceEqual', t => {
    ...
    const pets1 = new List([pet1, pet2]);
    const pets2 = new List([pet1, pet2]);
    const pets3 = new List([pet1]);
    t.true(pets1.SequenceEqual(pets2));
    t.false(pets1.SequenceEqual(pets3));
});
test('Single', t => {
    const fruits1 = new List();
    const fruits2 = new List(['orange']);
    const fruits3 = new List(['orange', 'apple']);
    ...
    t.throws(() => numbers1.Single(x => x === 5), /The collection does not contain exactly one element./);
    t.throws(() => numbers1.Single(x => x > 5), /The collection does not contain exactly one element./);
});
test('SingleOrDefault', t => {
    ...
    t.throws(() => numbers1.SingleOrDefault(x => x === 5), /The collection does not contain exactly one element./);
});
test('Skip', t => {
    const grades = new List([59, 82, 70, 56, 92, 98, 85]);
    t.deepEqual(grades
...
test('ToList', t => {
    t.deepEqual(new List([1, 2, 3]).ToList().ToArray(), [1, 2, 3]);
});
test('ToLookup', t => {
    // create a list of Packages
    const packages = new List([
        new Package({
    ...
    const lookup = packages.ToLookup(p => p.Company.substring(0, 1), p => p.Company + ' ' + p.TrackingNumber);
    const result = {
...
test('Union', t => {
    const ints1 = new List([5, 3, 9, 7, 5, 9, 3, 7]);
    const ints2 = new List([8, 3, 6, 4, 4, 9, 1, 0]);
    ...
    const store1 = new List([
        new Product({ Name: 'apple', Code: 9 }),
        new Product({ Name: 'orange', Code: 4 })
    ]);
    const store2 = new List([
        new Product({ Name: 'apple', Code: 9 }),
...
test('Zip', t => {
    ...
    const words = new List(['one', 'two', 'three']);
    t.deepEqual(numbers.Zip(words, (first, second) => `${first} ${second}`).ToArray(), ['1 one', '2 two', '3 three']);
    // larger second array
    const expected = ['one 1', 'two 2', 'three 3'];
    const numbers2 = new List([1, 2, 3, 4]);
    const words2 = new List(['one', 'two', 'three']);
    t.deepEqual(words2.Zip(numbers2, (first, second) => `${first} ${second}`).ToArray(), expected);
});
test('Where().Select()', t => {
